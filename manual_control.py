...
import re
import weakref
from Camaras.cam_man import DisplayManager, SensorManager
try:
    import pygame
...
class World(object):
    ...
    def restart(self, args):
        ...
        # Se ajusta la semilla para que la ejecución inicie siempre de la misma forma, en este caso un mini cooper de color naranja.
        random.seed(5);
    ...
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            self._camera_transforms = [
                # En esta sección de modifica el parámetro y y x para cambiar la posición en el mapa
                # se configura z para cambiar la altura, En la sección de rotación se cambia hacia donde mira la cámara
                # se pueden configurar los parámetros del pitch, roll y yaw.
                (carla.Transform(carla.Location(x=+0.1*bound_x, y=-0.2*bound_y, z=1.0*bound_z)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=+0.1*bound_x, y=-0.2*bound_y, z=1.0*bound_z), carla.Rotation(yaw=45.5)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=+0.1*bound_x, y=-0.2*bound_y, z=1.0*bound_z), carla.Rotation(yaw=-45.5)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=-0.2*bound_x, y=+0.0*bound_y, z=1.0*bound_z), carla.Rotation(yaw=180)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=+0.2*bound_x, y=+0.6*bound_y, z=1.0*bound_z), carla.Rotation(yaw=160)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=+0.2*bound_x, y=-0.6*bound_y, z=1.0*bound_z), carla.Rotation(yaw=-160)), Attachment.Rigid),
                (carla.Transform(carla.Location(x=-2.0*bound_x, y=+0.0*bound_y, z=2.0*bound_z), carla.Rotation(pitch=8.0)), Attachment.SpringArm)
                # (carla.Transform(carla.Location(x=-2.0*bound_x, y=+0.0*bound_y, z=2.0*bound_z), carla.Rotation(pitch=8.0)), Attachment.SpringArm),
                # (carla.Transform(carla.Location(x=+0.1*bound_x, y=-0.2*bound_y, z=1*bound_z)), Attachment.Rigid),
